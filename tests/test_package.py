...
class PackageTests(unittest.TestCase):
    ...
    def test_only_latest_ppt_is_included_and_premium_wins_same_date(self) -> None:
        for name in (
            "docs/地缘风险_产品介绍PPT_20260912.pptx",
            "docs/地缘风险_产品介绍PPT_20260913.pptx",
            "docs/地缘风险_产品介绍PPT_20260913_精美版.pptx",
        ):
            self.write_bytes(name, self.ppt_bytes("<slide><text>合成演示</text></slide>"))
        result = self.build()
        with zipfile.ZipFile(result.path) as archive:
            ppts = [name for name in archive.namelist() if name.endswith(".pptx")]
        self.assertEqual(ppts, ["docs/地缘风险_产品介绍PPT_20260913_精美版.pptx"])
