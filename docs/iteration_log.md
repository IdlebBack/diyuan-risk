...
- 团队成员统一为三位：林明强、陈治希、李宇欣；
- 更正旧材料中的姓名“陈冶希”为“陈治希”；
- 同步更新网页页脚、README、PPT 大纲、PPT 封面及全部页脚。

## 2026-09-14｜里程碑 7：精美化与 GitHub 最终集成

### 改动

- 网页新增统一品牌视觉系统：深海蓝、青蓝、风险橙与可信绿语义色；
  六页统一雷达 Hero、品牌侧栏、卡片、标签页、按钮、数据表、提示框和响应式布局；
- 事件库主表改为精简中文证据字段，隐藏演示工具栏，补充键盘焦点和减少动画偏好；
- 保留并合并 GitHub 最新的数据校验、图谱布局、过期结果失效和发布安全修复；
- PPT 重做为 16 页产品发布型演示：加入雷达主视觉、风险传导链、角色场景卡、
  AI/规则/人工责任矩阵、原生库存曲线、方案矩阵、双案例与证据成熟度流程；
- 打包脚本只选取最新日期的正式 PPT，同日优先 `_精美版`，避免提交包包含多个版本。

### 验证

- `python -m unittest discover -s tests -v`：83 项通过，2 项因当前 Windows
  环境不支持符号链接而跳过；
- `python scripts/cases.py` 与 `python scripts/demo.py`：退出码均为 0；
- Python 编译检查通过；Streamlit 六页逐页打开无异常；
- 精美版 PPT 16 页结构、字体、原生表格和原生图表校验通过。

## 2026-09-13｜重新克隆后的正确性与边界优化

### 基线与范围

从 GitHub 重新克隆，以 `52ff7de` 为基线开展本轮优化；原本地工作目录未覆盖。
不修改种子数据、不新增现实企业事实、不调用真实模型；已有提交和 PPT 保留。

### 已复现问题与修复

- 推演：移除“0% 削减按严重度猜成损失”的分支；事件交期取当前交期下限，
  EVT-04 不再把德国相机 16 周当前交期缩短为 14 周。
- 方案：新增库存改为增量追加，避免“+8 周”反而覆盖既有 +12 周库存；
  替代成本按引擎实际离散到货周数计入就绪周，输入参数做有限非负校验。
- 数据：增加启动校验、友好错误与 many-to-one 合并约束；允许空订单、可选在途表，
  保留原字符串编号（含前导零）和文本 NA，同订单同组件拆分行合并数量。
- 事件与评分：解除状态保持不变；负效果值不进入自动冲击；全零权重回默认，
  极大权重不溢出；主要因子改用加权贡献，默认暴露分不改变。
- 图谱：图例移至独立右侧区域，红色虚线图例对应实际关系；箭头和订单数量标签可读，
  供应商名称元数据纠正，集中度文案动态化。
- UI：事件、数据模式或底层 CSV 变化时清理旧并行结果与 AI 缓存；空巡检和输入变更
  清理旧信号，避免沿用旧批次勾选；数据错误显示修正指引而非堆栈。
- 测试：补核心边界、六页面 AppTest、图谱几何布局、导入失败原子性/批次隔离、
  发布白名单/凭据扫描/失败清理回归；CI 增加 Windows Python 3.12。

### 验收口径与限制

- 本地验证环境：Windows、Python 3.14.3、pandas 3.0.5、Streamlit 1.63.0、
  NetworkX 3.6.1、matplotlib 3.11.1。具体版本为本机记录，并非所有平台的锁文件。
- 案例 A 第 18 周、案例 B 第 17 周首次缺口及三批订单口径保持不变；默认暴露度
  DEP-02=71.5、DEP-01=52.5、DEP-03=39.5。
- 替代成本修正会改变方案表的示意金额：编码器 26 周窗口、18 周就绪、20% 溢价，
  成本为 258400 元示意（含第 18 周），不是现实报价或本轮新增商业事实。
- 符号链接真实创建专项在无权限机器上明确跳过；模拟 Windows reparse 属性校验仍执行。
- 本轮不承诺真实 LLM/RSS 连通性；生产排程、概率校准、冲突报道核验仍是既有局限。
...
- AI：配置状态不再冒充连通状态；超时、鉴权、限流、连接失败和非法 JSON
  分级提示，限制请求长度、输出长度与重试次数；失败不影响确定性推演；
- AI：抽取结果强制标记 `verify`，提示词明确“来源/日期/影响数值不由模型编造”；
  摘要与推演解读附来源、置信度和人工复核边界；解读按输入快照缓存；
- 数据与事件：默认使用固定种子数据；应用侧栏显式切换本地导入事件；案例页隔离本地事件，
  避免机器上的 RSS 导入改变评委看到的基线；新事件写入采用原子替换与进程内锁；
- 推演：库存周末恰好归零不再误报为当周缺口；累计缺口、窗口外订单、无效参数和低置信度事件处理更保守；
- 交付：增加 Markdown 推演报告下载、暴露度 CSV 下载、发布白名单/疑似密钥扫描、GitHub Actions 离线检查；
- 文档与回归：案例 A 改为第 18 周首次当周缺口（第 17 周末库存归零），案例 B 保持第 17 周；新增 6 项离线单元测试。

### 验证

- `python -m unittest discover -s tests -v`：6/6 PASS；
- `python scripts/cases.py`：案例 A–D 全部 PASS，退出码 0；
- `python scripts/demo.py`：固定种子数据冒烟通过，退出码 0；
- Streamlit 六个页面自动打开无异常；默认页面显示“固定模拟数据”与真实 AI 状态“已配置，尚未验证”。
- 首页增加可操作的验收路线与风险优先项；暴露度支持 CSV 导出，推演支持 Markdown 报告导出；
  AI 摘要显示来源链接与最近调用状态。
- 修复演示脚本旧交期文案、AI 来源链接字段和异常 URL 状态解析边界；补充 3 项 LLM/报告单元测试。
...
README 与提交包。学校与成员信息现统一更正为：西北民族大学、林明强、陈治希、李宇欣。
...
- 案例 A（编码器）与案例 B（芯片航运延误）在现有种子数据下第 17 周末库存归零，
  第 18 周首次出现当周缺口，
...
- 回归脚本中“第 18 周首次当周缺口”来自当前种子数据；若后续调整库存/交期/在途口径，
